...
function charCountColor(len: number) {
  if (len >= MAX_TEMPLATE_LENGTH) return "text-red-500";
  if (len >= 6500) return "text-orange-400";
    ...
    <div className="min-h-[calc(100vh-3rem)] bg-zinc-50 text-zinc-900 dark:bg-black dark:text-zinc-50">
      <div className="md:pl-56">
        <div className="flex min-h-[calc(100vh-3rem)] flex-col md:flex-row">
          ...
          {/* ── 왼쪽: 내 캐릭터 목록 ── */}
          <div className="w-full border-b border-zinc-200 px-6 py-10 md:w-1/2 md:border-b-0 md:border-r dark:border-zinc-800">
          ...
          <div className="w-full px-6 py-10 md:w-1/2">
            <header className="mb-6">
              ...
              <div className="flex items-baseline gap-2">
                <h2 className="text-lg font-semibold tracking-tight">프롬프트 템플릿</h2>
                {!loading && (
                  <span className="text-xs text-zinc-400 dark:text-zinc-500">{templates.length}/10</span>
                )}
              </div>
              {!showAddForm && (
                templates.length >= 10 ? (
              ...
              <p className="mt-1 text-sm text-zinc-400 dark:text-zinc-500">
                자주 쓰는 프롬프트를 템플릿으로 저장해두고, 캐릭터 제작 시 클릭 한 번으로 바로 불러오세요.
              </p>
            </header>
            ...
            {showAddForm && (
              <div className="mb-4 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                />
                <p className={`mb-2 text-right text-[10px] ${charCountColor(addContent.length)}`}>
                ...
                {templates.map((t) =>
                  editingId === t.id ? (
                    ...
                    <div key={t.id} className="rounded-xl border border-zinc-300 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                      ...
                      />
                      <p className={`mb-2 text-right text-[10px] ${charCountColor(editContent.length)}`}>
